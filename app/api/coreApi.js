...
if (config.noInmemoryRpcCache) {
	...
	miscCache = createMemoryLruCache(LRU(500));
	blockCache = createMemoryLruCache(LRU(500));
	txCache = createMemoryLruCache(LRU(2000));
}
		...
		tryCacheThenRpcApi(miscCache, "getRawMempool", 5000, rpcApi.getRawMempool).then(function(result) {
			...
			for (var txid in result) {
				...
				//var satoshiPerByte = feePerByte * 100000000;
                                var satoshiPerByte = feePerByte * 1;
				var age = Date.now() / 1000 - txMempoolInfo.time;
				var size = txMempoolInfo.size;
...
function getBlockByHeight(blockHeight) {
	return tryCacheThenRpcApi(blockCache, "getBlockByHeight-" + blockHeight, 3600000/5, function() {
...
function getBlockByHash(blockHash) {
	return tryCacheThenRpcApi(blockCache, "getBlockByHash-" + blockHash, 3600000/5, function() {
...
function getRawTransaction(txid) {
	...
	return tryCacheThenRpcApi(txCache, "getRawTransaction-" + txid, 3600000/5, rpcApiFunction, shouldCacheTransaction);
}

function getAddress(address) {
	return tryCacheThenRpcApi(miscCache, "getAddress-" + address, 3600000/5, function() {
